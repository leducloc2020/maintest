...
export const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(200)
        .json({ success: true, message: "Đã xóa tất cả các bản ghi có trùng userID." });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Không có bản ghi nào được xóa với userID đã cho." });
    }
  } catch (error) {
    console.error("Error deleting records:", error);
    return res
      .status(500)
      .json({ success: false, message: "Đã có lỗi xảy ra trong quá trình xóa." });
  }
};
...
export const ESPToDeleteBiometric = async (req, res, next) => {
  try {
    ...
    if (biometricIndexToDelete !== -1) {
      ...
      await User.updateOne({ userID: userId }, { $pull: { biometrics: { method: method } } });
  ...
  try {
    ...
    if (existingTimeInLog) {
      ...
      if (existingTimeOutLog) {
        ...
        const timeOutLocal = moment(timeOutUTC)
          .tz("Asia/Ho_Chi_Minh")
